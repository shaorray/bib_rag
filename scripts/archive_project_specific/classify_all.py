...
def main():
    ...
    print(f"to classify: {len(papers)} papers ({len(done_set)} already done, skipped)", file=sys.stderr)
    ...
    fh.close()
    print(f"done: {done} newly classified, {errs} errors, {time.time()-t0:.0f}s total", file=sys.stderr)
