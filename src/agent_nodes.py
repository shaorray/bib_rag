...
def rewrite_query(state: State, llm):
    ...
    if response.questions and response.is_clear:
        ...
        return {
            ...
            # HyDE probe: the hypothetical passage rides along to the agent
            # subgraphs via Send; each subgraph's mandatory first search
            # embeds question + passage jointly (answer-shaped text matches
            # answer-bearing chunks better than the question alone).
            "hydePassage": (response.hyde_passage or "").strip(),
        }
...
def orchestrator(state: AgentState, llm_with_tools):
    ...
    if not state.get("force_search_done", False):
        ...
        # HyDE (Hypothetical Document Embeddings, Gao et al. 2023): the
        # hypothetical answer passage from rewrite_query is injected as a
        # retrieval hint — the LLM is told to fold its terminology into the
        # first search query. Zero extra LLM calls (the passage was already
        # generated by the rewrite step); answer-shaped vocabulary matches
        # answer-bearing chunks better than question-shaped vocabulary.
        hyde = (state.get("hydePassage") or "").strip()
        if hyde and os.environ.get("HYDE", "1") != "0":
            human_msg = HumanMessage(
                content=f"{question}\n\n"
                        f"[HYPOTHETICAL PASSAGE — terminology hint for your "
                        f"first search query; NOT evidence, do NOT cite]\n{hyde}"
            )
        else:
