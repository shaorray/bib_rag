#!/usr/bin/env python3
"""
bib_rag_writer.py — Write-after-Grill academic paragraph writer.

Single paragraph composer of the toolkit (2026-08-31: merged from
bib_rag_grill.py, which had absorbed the original bib_rag_writer.py
template mode (--no-llm) and bib_rag_writer_debate.py (--debate); both
shims removed. Archived originals: scripts/archive_project_specific/).
...
Two-phase workflow inspired by Matt Pocock's /grill-me + /grill-with-docs:
  ...
  PHASE B: WRITE
    ...
    tissue filter, citation style). Backends: spec-aware LLM (default),
    --no-llm deterministic template, --debate relational analysis.

Dependency policy (pip-installable package): no odfpy, no scripts/
zotero_access. Output is plain-text/markdown (use --output <path.md>).
Citations are built from retrieval metadata (title/year/DOI) only.

Usage:
    # Interactive grilling (asks 1 question at a time, recommended)
    python3 -B bib_rag_writer.py "Eph drives boundary sharpening"

    # Auto-grill (uses LLM to fill in missing spec fields from CONTEXT.md)
    python3 -B bib_rag_writer.py "Eph drives boundary sharpening" --auto

    # Skip grill, use a saved spec from a prior run
    python3 -B bib_rag_writer.py --spec /tmp/bib_rag_grill_specs/grill_spec_20260607.json

    # Backend selection
    ... --no-llm   # deterministic template synthesis (original writer mode)
    ... --debate   # LLM analyzes passage relationships before synthesis

Why the grill pre-step exists (lessons from the original bib_rag_writer.py):
...
ZOTERO_BASE = "http://localhost:23119/api/users/0"  # informational only — not used
...
def search_bib_rag(query: str, top_k: int = 5) -> List[Dict]:
    ...
    for i in range(len(docs["ids"][0])):
        ...
        results.append({
            ...
            "authors": meta.get("authors", ""),   # real author string (chunk metadata)
            "year": meta.get("year", ""),
            "doi": meta.get("doi", ""),
...
# ---------------------------------------------------------------------------
# Output writer — plain-text/markdown (package-friendly; no odfpy)
# ---------------------------------------------------------------------------
def write_odt(paragraph: str, citations: List[str], spec: Dict, output_path: str):
    """Write the synthesis to `output_path` as plain text / markdown.

    (Name kept for backward compatibility with older scripts/commands; the
    output is no longer an ODT — odfpy was removed from the dependency
    policy. Use --output <path>.md to get a .md file; any extension works.)
    """
    lines = ["# Grill-spec Synthesis", ""]
    lines += [
        "[Grill spec — locked before search]",
        f"Topic: {spec.get('raw_topic', '')}",
        f"Mechanism focus: {spec.get('mechanism_focus', '—')}",
        f"Tissue: {spec.get('tissue_context', spec.get('tissue', '—'))}",
...
def main():
    ...
    parser.add_argument("--output", help="Output file path (default: <library>/outputs/writer_<ts>.md)")
    ...
    output = args.output or str(Path(_CFG["outputs_dir"]) / f"writer_{ts}.md")
...
def _inline_citation(passage: Dict) -> tuple:
    """(inline_cite, year, full_ref) — built from retrieval metadata only
    (title / year / DOI as stored by bib_rag indexing; no Zotero API call —
    zotero_access was removed from the dependency policy). Title-author
    prefixes ('Author et al. - Year - Title') are parsed as the fallback
    that was always there for Zotero misses; now the primary path."""
    ref_title = _clean_title(passage["title"])
    ref_year_m = re.search(r"\b(19\d{2}|20\d{2})\b", passage["title"])

    title = passage["title"]
    authors = passage.get("authors", "") or ""
    year = (ref_year_m.group(1) if ref_year_m else "") or passage.get("year") or "n.d."
    if authors:
        # Author-string formats seen across libraries (chunk metadata is
        # normalized inconsistently; parent_store keeps 'Last, F; Last2, F2'):
        #   "Barrios A; Poole RJ; Durbin L"          (Last + initials, ';'-sep)
        #   "Paulson,Alicia F.; Fang,Xiang; Ji,Hong"  (Last,First; ... comma-pair)
        #   "Li, Bin, Zhang, Wen, ..."                (flattened comma list)
        # Rule: split on ';' first; if one segment → split on ','; the
        # SURNAME is the first token of the chosen segment (never an
        # initial — initials are short/uppercase; a surname token is the
        # longest alphabetic token).
        seg = authors.split(";")[0].strip()
        if "," in seg and len(authors.split(";")) == 1:
            # flattened comma list: surname, given-name, surname2, given2...
            tokens = [t.strip() for t in seg.split(",") if t.strip()]
            surname = tokens[0] if tokens else "Unknown"
        else:
            tokens = [t for t in seg.replace(",", " ").split() if t]
            # drop trailing initials AFTER the first token — never strip
            # position 0: short surnames ("Lu", "Qi", "Ma") are 2 letters
            # and uppercase too; an initial only ever follows a surname.
            # Initial shape: single letter ("P") or dot-suffixed ("F.", "R.J.")
            def _is_initial(tok: str) -> bool:
                return (len(tok) == 1
                        or (len(tok) <= 4 and tok.endswith(".")
                            and tok.replace(".", "").isupper()))
            while len(tokens) > 1 and _is_initial(tokens[-1]):
                tokens = tokens[:-1]
            surname = tokens[0] if tokens else "Unknown"
        multi = len(authors.split(";")) > 1 or "," in authors
        inline = f"{surname} et al." if multi else surname
    elif " et al." in title:
